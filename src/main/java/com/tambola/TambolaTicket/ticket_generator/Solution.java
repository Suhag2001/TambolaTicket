...
public class Solution {
    HashMap<String, List<List<Integer>>> tambolaTickets = new HashMap<>();

    public HashMap<String, List<List<Integer>>> generateTambolaSets() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                int randNumIndex = Random.getRand(0, li.size() - 1);
        ...
        int randNumIndex = Random.getRand(0, lastCol.size() - 1);
        int randNum = lastCol.get(randNumIndex);

        int randSetIndex = Random.getRand(0, sets.size() - 1);
        ...
        for (int pass = 0; pass < 3; pass++) {
            for (int i = 0; i < 9; i++) {
                ...
                int randNumIndex_p = Random.getRand(0, col.size() - 1);
                int randNum_p = col.get(randNumIndex_p);

                boolean vacantSetFound = false;
                while (!vacantSetFound) {
                    int randSetIndex_p = Random.getRand(0, sets.size() - 1);
                    List<List<Integer>> randSet_p = sets.get(randSetIndex_p);

                    if (ElementChecker.getNumberOfElementsInSet(randSet_p) == 15 || randSet_p.get(i).size() == 2)
                        continue;
        ...
        for (int i = 0; i < 9; i++) {
            ...
            while (!vacantSetFound) {
                ...
                if (ElementChecker.getNumberOfElementsInSet(randSet_p) == 15 || randSet_p.get(i).size() == 3)
                    continue;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            tambolaTickets.put(String.valueOf(++number), ticketList);
        }
